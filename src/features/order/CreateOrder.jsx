...
import { createOrder } from "../../services/apiCafe";
import EmptyCart from '../cart/EmptyCart'
import Button from "../../ui/Button";
import store from '../../store'
import { formatCurrency } from "../../utils/helpers";
import { fetchAddress, getUser } from "../address/addressSlice";
...
function CreateOrder() {
  ...
  const { status: addressStatus, position, address, error: errorAddress } = useSelector(getUser)
  ...
  return (
    <div className='px-4 py-6'>
      <h2 className='text-xl font-semibold mb-8'>Ready to order? Let's start!</h2>
      ...
      <Form method='POST'>
        ...
        <div className='mb-5 flex gap-2 flex-col sm:flex-row sm:items-center'>
          <label className='sm:basis-40'>Add commentary</label>
          <div className='grow'>
            <input className='input w-full' type='text' name='comment' id='comment' value={comment} onChange={(e) => setComment(e.target.value)} />
          </div>
        </div>

        <div className='mb-12 flex gap-5 items-center'>
          <input
            className='h-6 w-6 accent-emerald-400 focus:outline-none focus:ring focus:ring-emerald-400 focus:ring-offset-2'
            type='checkbox'
            name='priority'
            id='priority'
            value={withPriority}
...
export async function action({ request }) {
  ...
  const totalSumOfCart = cart.reduce((sum, item) => sum + item.totalPrice, 0)
  ...
  const order = {
    ...
    orderPrice: totalSumOfCart,
    priorityPrice: data.priority === 'true' ? totalSumOfCart * 0.2 : 0,
    status: "preparing",
  }
